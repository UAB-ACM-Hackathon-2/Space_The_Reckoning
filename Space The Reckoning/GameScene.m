...
@property (nonatomic, strong) NSMutableDictionary *saveFile;
@end
...
- (void) setupBackGroundSong
{
    ...
    NSString *filePath = [[NSBundle mainBundle] pathForResource:@"save" ofType:@"plist"];
    if (![manager fileExistsAtPath:@"./save.plist"])
    {
        NSData *temp = [NSData dataWithContentsOfFile:filePath];
        [manager createFileAtPath:@"./save.plist" contents:temp attributes:nil];
    }
    self.saveFile = [NSMutableDictionary dictionaryWithContentsOfFile:@"./save.plist"];
}
                          ...
                          [SKAction sequence:@[
                                               [SKAction performSelector:
                                                @selector(createBadGuys)
                                                                onTarget:self],
                                               [SKAction waitForDuration:0.5 withRange:2.5]
                                                                            ]]];
...
- (void) subtractHealth
{
    [self.saveFile setObject:[NSNumber numberWithInt:HighScore] forKey:@"HighScore"];
    [self.saveFile setObject:[NSNumber numberWithInt:doritoCount] forKey:@"doritoPoints"];
    [self.saveFile writeToFile:@"./save.plist" atomically:YES];
    Health--;
    if (Health <= 0)
    {
        [self.background_music stop];
